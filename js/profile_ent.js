...
let badgeList = ["data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABYAAAAdCAYAAACuc5z4AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAABQSURBVHgB7dOxCUAhDATQ+x87e1dxIQewt3Y4V7G3N4KCA0REuAdHuiMQAhC975vTSTJ0JEn9Qaet41mJh44iaWYrjtARRvGxjUELX5rohg6RewnhdtmsRQAAAABJRU5ErkJggg=="
];   // 뱃지 받아와서 리스트에 저장
...
function showBadge() {
  let template=``;
  const BadgePlus = document.querySelector('.add_badge');
  let open = document.querySelector('.open_modal');
  console.log(BadgePlus);
  console.log(badgeListLength);

  if(badgeListLength != 0) {
    for(let i = 0; i < badgeListLength; i++) {
      template += `
      <div class="badge" id="badge${i+1}"><img class="open_modal" onclick="modal_On(${i})" src="${badgeList[i]}"></div>
      `
    }
    template += `
    <div class="add_badge"><img src="../image/Plus Math.png" ></div>
    `
